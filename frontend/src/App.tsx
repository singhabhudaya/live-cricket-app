...
/** ----------- API/WS base helpers (robust to misconfigured envs) ----------- */
function normalizeApiBase(raw?: string) {
  // prefer env, else default to your Render backend
  let base = (raw || 'https://live-cricket-app.onrender.com').replace(/\/+$/, '');
  // ensure trailing /api once
...
// Build API URLs safely (always under /api)
const API = (p: string) => join(API_BASE, p);

// WS: use explicit env if provided; else derive from API base by stripping /api and forcing wss in prod
function deriveWsBase(): string {
  const env = import.meta.env.VITE_WS_URL as string | undefined;
  if (env) return env.replace(/\/+$/, '');
  // from API_BASE -> remove trailing /api
  let httpBase = API_BASE.replace(/\/api$/i, '');
  // ensure https -> wss, http -> ws
  try {
    const u = new URL(httpBase);
    u.protocol = u.protocol === 'http:' ? 'ws:' : 'wss:';
    return u.toString().replace(/\/+$/, '');
  } catch {
    return 'wss://live-cricket-app.onrender.com';
  }
}
const WS = `${deriveWsBase()}/ws`;
